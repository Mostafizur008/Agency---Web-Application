...
<div class="row">
                ...
                <table id="demo-foo-filtering" class="table table-bordered table-centered mb-0" data-page-size="7">
                    ...
                    <tbody>
                        <tr align="center">
                            ...
                            <td>
                                @if($channel->status == '1')
                                <a href="{{ route('ct.status',$channel->id)}}" class="btn btn-xs btn-primary">Active</a>
                                @else
                                <a href="{{ route('ct.status',$channel->id)}}" class="btn  btn-xs btn-danger">Inactive</a>
                                @endif
                            </td>
                            <td>
                                <a type="button" href="{{route('ct.edit',$channel->id)}}" class="btn btn-xs btn-primary"><i class="fe-edit"></i></a>
    ...
    <div class="col-lg-5">
        ...
        <div class="card-box">
            <form action="{{ isset($editData) ? route('ct', $editData->id) : route('ct.store') }}" method="post" enctype="multipart/form-data">
                @csrf
                @if(isset($editData))
                    @method('PUT')
                @endif
...
@include('backend.code-section.ajax-script.live.category')
